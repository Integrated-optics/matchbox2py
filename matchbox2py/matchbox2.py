...
from matchbox2py.command_parser import CommandParser
from matchbox2py.laser_commands import LaserCommands
from matchbox2py.models.laser_on_port import LaserOnPort
...
class MatchBox2Laser:
    ...
    def get_laser_info(self):
        ...
        CommandParser.parse_error_message(response)
        return CommandParser.parse_laser_info(response)

    def get_laser_readings(self):
        self._send_message(LaserCommands.receive_readings())
        response = self._get_message()
        CommandParser.parse_error_message(response)
        return CommandParser.parse_laser_readings(response)

    def get_laser_settings(self):
        self._send_message(LaserCommands.receive_settings())
        response = self._get_message()
        CommandParser.parse_error_message(response)
        return CommandParser.parse_laser_settings(response)
    ...
    def is_laser_on_port(self):
        return bool(self.get_laser_info())
        ...
        for port in available_ports:
            ...
            try:
                ...
                if self.is_laser_on_port():
                    ...
                    laser = LaserOnPort(portName=port.device,model=laser_info.model,serial=laser_info.serial_no,firmware=laser_info.firmware)
